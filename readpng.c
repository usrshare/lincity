...
void user_warning_fn(png_structp png_ptr, png_const_charp warning_msg)
{
	...
	int
load_png_graphics (char *txt_filename, char *png_filename, uint32_t* out_palette)
{
	png_uint_32 row;
	png_structp png_ptr;
	png_infop info_ptr;
	...
	png_get_PLTE(png_ptr,info_ptr,&imgpal,&palsize);

	if (out_palette) {

	for (int i=0; i < 256; i++) {
		png_color* col = &imgpal[i];
	out_palette[i] = (col->red * 65536) + (col->green * 256) + (col->blue);
	}
